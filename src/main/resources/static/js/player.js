	...
	var playlist = [];
		...
		if (result.code==200){
			...
			for (var i = 0; i < playlist.length; i++){
				var item = playlist[i];
				$('#playlist').append('<li class="lib" style="overflow:hidden;"><strong style="margin-left: 5px;">'+item.name+'</strong><span style="float: right;" class="artist">'+item.singer+'</span></li>');
				if (item.url == "") {
			...
			var loadMusic = function(i){
				...
				var newaudio = $('<audio>').html('<source src="'+item.url+'"><source src="'+item.ogg+'">').appendTo('#player');
				$('.cover').html('<img src="'+item.cover+'" alt="'+item.album+'">');
				$('.musicTag').html('<strong>'+item.name+'</strong><span> - </span><span class="artist">'+item.singer+'</span>');
